...
namespace TemplateTools.ConApp
{
    partial class Program
    {
            ...
            while (input.Equals("x") == false)
            {
                ...
                if (Int32.TryParse(input, out var select))
                {
                    if (select == 1)
                    {
                        ...
                        var qtSolutions = Program.GetQTTankpreiseSolutions(Program.UserPath).Union(new[] { solutionPath }).ToArray();
        ...
        internal static string[] GetQTTankpreiseSolutions(string startPath)
        {
            var result = new List<string>();
            var qtPaths = GetQTTankpreiseProjects(startPath);
        ...
        internal static string[] GetQTTankpreiseProjects(string startPath)
        {
            return QueryDirectoryStructure(startPath, n => n.StartsWith("QT"), "bin", "obj", "node_modules");
        }
        internal static string[] GetQTTankpreisePaths(string startPath, params string[] includePaths)
        {
            var qtProjects = GetQTTankpreiseProjects(startPath).Union(includePaths).ToArray();
        ...
        internal static string[] GetQTTankpreiseParentPaths(string startPath, params string[] includePaths)
        {
            var result = new List<string>();
            var qtProjects = GetQTTankpreiseProjects(startPath).Union(includePaths).ToArray();
            var qtPaths = qtProjects.Select(p => Program.GetParentDirectory(p))
